...
int Reader(void * data)
{
	while (true)
	{
		...
		std::ifstream myfile("mythreads.txt");//ifstream is used for writng and reading
		std::string line;
...
int Writer(void * data)
{
	...
	while (true)
	{
		...
		std::ofstream myfile;
		myfile.open("mythreads.txt");//Open text file
		myfile << "Thread: " << i << std::endl; //Output to text file
